...
class NikeScraper(BaseProductScraper):
    store_name = "Nike Tienda Oficial"
    brand = "Nike"
    base_url = "https://www.nike.com.co/"

    def _build_search_url(self, query: str, page: int) -> str:
        encoded = quote_plus(query.strip())
        if page <= 1:
            return f"{self.base_url}/co/w?q={encoded}"
        return f"{self.base_url}/co/w?q={encoded}&page={page}"
    ...
    def _extract_products_from_page(self, page: Any, url: str, limit: int = 50) -> list[dict[str, Any]]:
        ...
        cards = self._css(page, ".product-card, .product-card__body")
        seen = set()
        for card in cards[:limit]:
            try:
                title_node = first(self._css(card, ".product-card__title"))
                price_node = first(self._css(card, ".product-price, .product-card__price"))
                a_node = first(self._css(card, "a"))
                img_node = first(self._css(card, "img"))

                title = title_node.text.strip() if title_node and getattr(title_node, 'text', None) else None
                ...
                if img_node is not None:
                    image_src = img_node.attrib.get("src") or img_node.attrib.get("data-src")
